...
def checkInLuaDataBase():
    ...
    if utils.pushd( _JavaConfigGenPath ):
        cmd = 'call ' + 'excelToJava.bat'
        os.system( cmd )
        utils.popd()
    ...
    if utils.queryYesNo('Upload to SVN Server ?'):
        ...
        print( 'commit %s' % _ServerJavaDataBasePath2 )
        svn.checkin( _ServerJavaDataBasePath2, 'PythonTool: upload server config' )
