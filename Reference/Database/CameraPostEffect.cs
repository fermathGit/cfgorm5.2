...
namespace Database {
	public partial class CameraPostEffect {
		const string DBName = "camera_posteffect";
		#region members
		private int m_id = 0;
		private string m_colorname = String.Empty;
		private float m_contrast = 0;
		private int m_curve_b = 0;
		private int m_curve_g = 0;
		private int m_curve_r = 0;
		private float m_saturation = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, CameraPostEffect> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, CameraPostEffect>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public string colorname { get { return m_colorname; } }
		public float contrast { get { return m_contrast; } }
		public int curve_b { get { return m_curve_b; } }
		public int curve_g { get { return m_curve_g; } }
		public int curve_r { get { return m_curve_r; } }
		public float saturation { get { return m_saturation; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static CameraPostEffect __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static CameraPostEffect() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( CameraPostEffect ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, CameraPostEffect> idCacheData {
		...
		static CameraPostEffect ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new CameraPostEffect();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_colorname = __mask[ 0 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_colorname, 9 ) : __default.m_colorname;
				temp.m_contrast = __mask[ 1 ] ? q.GetFloat( DBCache.FID_contrast, 8 ) : __default.m_contrast;
				temp.m_curve_b = __mask[ 2 ] ? q.GetInt( DBCache.FID_curve_b, 7 ) : __default.m_curve_b;
				temp.m_curve_g = __mask[ 3 ] ? q.GetInt( DBCache.FID_curve_g, 7 ) : __default.m_curve_g;
				temp.m_curve_r = __mask[ 4 ] ? q.GetInt( DBCache.FID_curve_r, 7 ) : __default.m_curve_r;
				temp.m_saturation = __mask[ 5 ] ? q.GetFloat( DBCache.FID_saturation, 10 ) : __default.m_saturation;
				return temp;
			}
			return null;
		}

 		public static CameraPostEffect Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			CameraPostEffect ret = null;
		...
		const int VersionHash = 0x2a711e69;
		#endregion
	}
}
//EOF
