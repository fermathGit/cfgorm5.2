...
namespace Database {
	public partial class Pet_Equip {
		const string DBName = "pet_equip";
		#region members
		private int m_id = 0;
		private int m_degree = 0;
		private int m_itemDataId = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Pet_Equip> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Pet_Equip>();
		#endregion
		#region properties
		...
		static Pet_Equip __default = null;
		static int __hash = 0;
		...
		static Pet_Equip() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Pet_Equip ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Pet_Equip> idCacheData {
		...
		static Pet_Equip ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Pet_Equip();
				...
				temp.m_itemDataId = __mask[ 2 ] ? q.GetInt( DBCache.FID_itemDataId, 10 ) : __default.m_itemDataId;
				temp.m_level = __mask[ 3 ] ? q.GetInt( DBCache.FID_level, 5 ) : __default.m_level;
				temp.m_part = __mask[ 5 ] ? q.GetInt( DBCache.FID_part, 4 ) : __default.m_part;
				temp.m_quilaty = __mask[ 6 ] ? q.GetInt( DBCache.FID_quilaty, 7 ) : __default.m_quilaty;
				Pet_EquipEx.IsValid_attes = __mask[ 0 ];
				Pet_EquipEx.IsValid_needItemes = __mask[ 4 ];
				if ( Pet_EquipEx.onLoadComplexField_attes != null ) {
					Pet_EquipEx.onLoadComplexField_attes( temp, q );
				}
				if ( Pet_EquipEx.onLoadComplexField_needItemes != null ) {
					Pet_EquipEx.onLoadComplexField_needItemes( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static Pet_Equip Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Pet_Equip ret = null;
		...
		const int VersionHash = 0x69f81a22;
		#endregion
	}

	public partial class Pet_EquipEx {
		public delegate void LoadComplexField( Pet_Equip data, LuaDatabase.Query q );
		...
		internal static bool IsValid_needItemes = true;
		static LoadComplexField OnLoadComplexField_needItemes = null;
		public static LoadComplexField onLoadComplexField_needItemes {
			get {
				return OnLoadComplexField_needItemes;
			}
		}
	}
}
