...
namespace Database {
	public partial class Pet_Practice {
		const string DBName = "pet_practice";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Pet_Practice> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Pet_Practice>();
		#endregion
		#region properties
		...
		static Pet_Practice __default = null;
		static int __hash = 0;
		...
		static Pet_Practice() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Pet_Practice ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Pet_Practice> idCacheData {
		...
		static Pet_Practice ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Pet_Practice();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_degree = __mask[ 0 ] ? q.GetInt( DBCache.FID_degree, 6 ) : __default.m_degree;
				temp.m_hole = __mask[ 1 ] ? q.GetInt( DBCache.FID_hole, 4 ) : __default.m_hole;
				temp.m_PropType = __mask[ 2 ] ? q.GetInt( DBCache.FID_PropType, 8 ) : __default.m_PropType;
				return temp;
			}
			return null;
		}

 		public static Pet_Practice Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Pet_Practice ret = null;
		...
		const int VersionHash = 0x46199d4c;
		#endregion
	}
}
//EOF
