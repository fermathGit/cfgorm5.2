...
namespace Database {
	public partial class AchievementDaily {
		const string DBName = "achievementDaily";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, AchievementDaily> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, AchievementDaily>();
		#endregion
		#region properties
		...
		static AchievementDaily __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static AchievementDaily() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( AchievementDaily ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, AchievementDaily> idCacheData {
		...
		static AchievementDaily ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new AchievementDaily();
				...
				temp.m_num = __mask[ 2 ] ? q.GetInt( DBCache.FID_num, 3 ) : __default.m_num;
				return temp;
			}
			return null;
		}

 		public static AchievementDaily Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			AchievementDaily ret = null;
		...
		const int VersionHash = 0x56cda1e8;
		#endregion
	}
}
//EOF
