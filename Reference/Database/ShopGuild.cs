...
namespace Database {
	public partial class ShopGuild {
		const string DBName = "shop_guild";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, ShopGuild> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, ShopGuild>();
		#endregion
		#region properties
		...
		static ShopGuild __default = null;
		static int __hash = 0;
		...
		static ShopGuild() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( ShopGuild ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, ShopGuild> idCacheData {
		...
		static ShopGuild ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new ShopGuild();
 		...
 		public static ShopGuild Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			ShopGuild ret = null;
