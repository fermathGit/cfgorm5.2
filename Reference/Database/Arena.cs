...
namespace Database {
	public partial class Arena {
		const string DBName = "arena";
		#region members
		private int m_id = 0;
		private int m_next_rank = 0;
		private int m_pos_max = 0;
		private int m_pos_min = 0;
		private string m_rankname = String.Empty;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, Arena> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Arena>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public int next_rank { get { return m_next_rank; } }
		public int pos_max { get { return m_pos_max; } }
		public int pos_min { get { return m_pos_min; } }
		public string rankname { get { return m_rankname; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static Arena __default = null;
		static int __hash = 0;
		...
		static Arena() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Arena ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Arena> idCacheData {
		...
		static Arena ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Arena();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_next_rank = __mask[ 1 ] ? q.GetInt( DBCache.FID_next_rank, 9 ) : __default.m_next_rank;
				temp.m_pos_max = __mask[ 2 ] ? q.GetInt( DBCache.FID_pos_max, 7 ) : __default.m_pos_max;
				temp.m_pos_min = __mask[ 3 ] ? q.GetInt( DBCache.FID_pos_min, 7 ) : __default.m_pos_min;
				temp.m_rankname = __mask[ 4 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_rankname, 8 ) : __default.m_rankname;
				ArenaEx.IsValid_first_reward_item = __mask[ 0 ];
				if ( ArenaEx.onLoadComplexField_first_reward_item != null ) {
					ArenaEx.onLoadComplexField_first_reward_item( temp, q );
				}
				return temp;
			}
			return null;
		...
		const int VersionHash = 0x4cbe1869;
		#endregion
	}

	public partial class ArenaEx {
		public delegate void LoadComplexField( Arena data, LuaDatabase.Query q );
		internal static bool IsValid_first_reward_item = true;
		static LoadComplexField OnLoadComplexField_first_reward_item = null;
		public static LoadComplexField onLoadComplexField_first_reward_item {
			get {
				return OnLoadComplexField_first_reward_item;
			}
		}
	}
}
