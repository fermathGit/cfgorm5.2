...
namespace Database {
	public partial class Exp_offline {
		const string DBName = "exp_offline";
		#region members
		private int m_id = 0;
		private int m_exp_free = 0;
		private int m_exp_glod = 0;
		private int m_item = 0;
		private int m_level_max = 0;
		private int m_level_min = 0;
		private int m_money_num = 0;
		private int m_money_type = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, Exp_offline> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Exp_offline>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public int exp_free { get { return m_exp_free; } }
		public int exp_glod { get { return m_exp_glod; } }
		public int item { get { return m_item; } }
		public int level_max { get { return m_level_max; } }
		public int level_min { get { return m_level_min; } }
		public int money_num { get { return m_money_num; } }
		public int money_type { get { return m_money_type; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static Exp_offline __default = null;
		static int __hash = 0;
		...
		static Exp_offline() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Exp_offline ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Exp_offline> idCacheData {
		...
		static Exp_offline ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Exp_offline();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_exp_free = __mask[ 0 ] ? q.GetInt( DBCache.FID_exp_free, 8 ) : __default.m_exp_free;
				temp.m_exp_glod = __mask[ 1 ] ? q.GetInt( DBCache.FID_exp_glod, 8 ) : __default.m_exp_glod;
				temp.m_item = __mask[ 2 ] ? q.GetInt( DBCache.FID_item, 4 ) : __default.m_item;
				temp.m_level_max = __mask[ 3 ] ? q.GetInt( DBCache.FID_level_max, 9 ) : __default.m_level_max;
				temp.m_level_min = __mask[ 4 ] ? q.GetInt( DBCache.FID_level_min, 9 ) : __default.m_level_min;
				temp.m_money_num = __mask[ 5 ] ? q.GetInt( DBCache.FID_money_num, 9 ) : __default.m_money_num;
				temp.m_money_type = __mask[ 6 ] ? q.GetInt( DBCache.FID_money_type, 10 ) : __default.m_money_type;
				return temp;
			}
			return null;
		}

 		public static Exp_offline Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Exp_offline ret = null;
		...
		const int VersionHash = 0x22d17244;
		#endregion
	}
}
//EOF
