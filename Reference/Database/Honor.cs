...
namespace Database {
	public partial class Honor {
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Honor> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Honor>();
		#endregion
		#region properties
		...
		static Honor() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Honor ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Honor> idCacheData {
		...
		static Honor ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Honor();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_fighting = __mask[ 0 ] ? q.GetInt( DBCache.FID_fighting, 8 ) : __default.m_fighting;
				temp.m_name = __mask[ 2 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_name, 4 ) : __default.m_name;
				HonorEx.IsValid_item = __mask[ 1 ];
				HonorEx.IsValid_prop = __mask[ 3 ];
				if ( HonorEx.onLoadComplexField_item != null ) {
					HonorEx.onLoadComplexField_item( temp, q );
				}
				if ( HonorEx.onLoadComplexField_prop != null ) {
					HonorEx.onLoadComplexField_prop( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static Honor Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Honor ret = null;
		...
		const int VersionHash = 0x1b3e87a8;
		#endregion
	}

	public partial class HonorEx {
		public delegate void LoadComplexField( Honor data, LuaDatabase.Query q );
		internal static bool IsValid_item = true;
		static LoadComplexField OnLoadComplexField_item = null;
		public static LoadComplexField onLoadComplexField_item {
			get {
				return OnLoadComplexField_item;
			}
		}
		internal static bool IsValid_prop = true;
		static LoadComplexField OnLoadComplexField_prop = null;
		public static LoadComplexField onLoadComplexField_prop {
			get {
				return OnLoadComplexField_prop;
			}
		}
	}
}
