...
namespace Database {
	public partial class VipPower {
		const string DBName = "vip_power";
		#region members
		private int m_id = 0;
		private int m_level = 0;
		private int m_param1 = 0;
		private int m_param2 = 0;
		private int m_param3 = 0;
		private int m_power = 0;
		private int m_sub_power = 0;
		private string m_txt = String.Empty;
		private int m_type = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, VipPower> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, VipPower>();
		#endregion
		#region properties
		...
		static VipPower() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( VipPower ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, VipPower> idCacheData {
		...
		static VipPower ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new VipPower();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_level = __mask[ 0 ] ? q.GetInt( DBCache.FID_level, 5 ) : __default.m_level;
				temp.m_param1 = __mask[ 1 ] ? q.GetInt( DBCache.FID_param1, 6 ) : __default.m_param1;
				temp.m_param2 = __mask[ 2 ] ? q.GetInt( DBCache.FID_param2, 6 ) : __default.m_param2;
				temp.m_param3 = __mask[ 3 ] ? q.GetInt( DBCache.FID_param3, 6 ) : __default.m_param3;
				temp.m_power = __mask[ 4 ] ? q.GetInt( DBCache.FID_power, 5 ) : __default.m_power;
				temp.m_sub_power = __mask[ 5 ] ? q.GetInt( DBCache.FID_sub_power, 9 ) : __default.m_sub_power;
				temp.m_txt = __mask[ 6 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_txt, 3 ) : __default.m_txt;
 		...
 		public static VipPower Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			VipPower ret = null;
		...
		const int VersionHash = 0x67b204f7;
		#endregion
	}
}
//EOF
