...
namespace Database {
	public partial class Guild_salary {
		const string DBName = "guild_salary";
		#region members
		private int m_id = 0;
		private int m_addPer = 0;
		private int m_max = 0;
		private int m_min = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Guild_salary> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Guild_salary>();
		#endregion
		#region properties
		...
		static Guild_salary __default = null;
		static int __hash = 0;
		...
		static Guild_salary() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Guild_salary ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Guild_salary> idCacheData {
		...
		static Guild_salary ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Guild_salary();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_addPer = __mask[ 0 ] ? q.GetInt( DBCache.FID_addPer, 6 ) : __default.m_addPer;
				temp.m_max = __mask[ 1 ] ? q.GetInt( DBCache.FID_max, 3 ) : __default.m_max;
				temp.m_min = __mask[ 2 ] ? q.GetInt( DBCache.FID_min, 3 ) : __default.m_min;
 		...
 		public static Guild_salary Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Guild_salary ret = null;
		...
		const int VersionHash = 0x79207b46;
		#endregion
	}
}
//EOF
