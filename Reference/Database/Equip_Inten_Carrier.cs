...
namespace Database {
	public partial class Equip_Inten_Carrier {
		const string DBName = "equip_inten_carrier";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Equip_Inten_Carrier> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Equip_Inten_Carrier>();
		#endregion
		#region properties
		...
		static Equip_Inten_Carrier __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static Equip_Inten_Carrier() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Equip_Inten_Carrier ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Equip_Inten_Carrier> idCacheData {
		...
		static Equip_Inten_Carrier ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Equip_Inten_Carrier();
				...
				temp.m_def_mag = __mask[ 2 ] ? q.GetInt( DBCache.FID_def_mag, 7 ) : __default.m_def_mag;
				temp.m_def_phy = __mask[ 3 ] ? q.GetInt( DBCache.FID_def_phy, 7 ) : __default.m_def_phy;
				temp.m_hpmax = __mask[ 4 ] ? q.GetInt( DBCache.FID_hpmax, 5 ) : __default.m_hpmax;
				return temp;
			}
			return null;
		}

 		public static Equip_Inten_Carrier Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Equip_Inten_Carrier ret = null;
