...
namespace Database {
	public partial class TaskMaster {
		const string DBName = "task_master";
		#region members
		private int m_id = 0;
		private int m_masterReward = 0;
		private int m_maxLv = 0;
		private int m_minLV = 0;
		private int m_num = 0;
		private int m_rewardGroupId = 0;
		private int m_taskGroupId = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, TaskMaster> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, TaskMaster>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public int masterReward { get { return m_masterReward; } }
		...
		static TaskMaster __default = null;
		static int __hash = 0;
		...
		static TaskMaster() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( TaskMaster ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, TaskMaster> idCacheData {
		...
		static TaskMaster ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new TaskMaster();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_masterReward = __mask[ 0 ] ? q.GetInt( DBCache.FID_masterReward, 12 ) : __default.m_masterReward;
				temp.m_maxLv = __mask[ 1 ] ? q.GetInt( DBCache.FID_maxLv, 5 ) : __default.m_maxLv;
				temp.m_minLV = __mask[ 2 ] ? q.GetInt( DBCache.FID_minLV, 5 ) : __default.m_minLV;
				temp.m_num = __mask[ 3 ] ? q.GetInt( DBCache.FID_num, 3 ) : __default.m_num;
				temp.m_rewardGroupId = __mask[ 5 ] ? q.GetInt( DBCache.FID_rewardGroupId, 13 ) : __default.m_rewardGroupId;
				temp.m_taskGroupId = __mask[ 6 ] ? q.GetInt( DBCache.FID_taskGroupId, 11 ) : __default.m_taskGroupId;
				TaskMasterEx.IsValid_reward_item = __mask[ 4 ];
				if ( TaskMasterEx.onLoadComplexField_reward_item != null ) {
					TaskMasterEx.onLoadComplexField_reward_item( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static TaskMaster Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			TaskMaster ret = null;
		...
		const int VersionHash = 0x44f4de26;
		#endregion
	}

	public partial class TaskMasterEx {
		public delegate void LoadComplexField( TaskMaster data, LuaDatabase.Query q );
		internal static bool IsValid_reward_item = true;
		static LoadComplexField OnLoadComplexField_reward_item = null;
		public static LoadComplexField onLoadComplexField_reward_item {
			get {
				return OnLoadComplexField_reward_item;
			}
		}
	}
}
