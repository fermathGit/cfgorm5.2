...
namespace Database {
	public partial class TalkToPlayer {
		const string DBName = "talktoplayer";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, TalkToPlayer> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, TalkToPlayer>();
		#endregion
		#region properties
		...
		static TalkToPlayer __default = null;
		static int __hash = 0;
		...
		static TalkToPlayer() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( TalkToPlayer ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, TalkToPlayer> idCacheData {
		...
		static TalkToPlayer ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new TalkToPlayer();
 		...
 		public static TalkToPlayer Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			TalkToPlayer ret = null;
		...
		const int VersionHash = 0x74322696;
		#endregion
	}
}
//EOF
