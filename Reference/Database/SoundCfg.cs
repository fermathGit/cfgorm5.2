...
namespace Database {
	public partial class SoundCfg {
		const string DBName = "SoundCfg";
		#region members
		private int m_id = 0;
		private string m_res = String.Empty;
		private string m_sound = String.Empty;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, SoundCfg> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, SoundCfg>();
		#endregion
		#region properties
		...
		static SoundCfg __default = null;
		static int __hash = 0;
		...
		static SoundCfg() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( SoundCfg ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, SoundCfg> idCacheData {
		...
		static SoundCfg ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new SoundCfg();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_res = __mask[ 0 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_res, 3 ) : __default.m_res;
				temp.m_sound = __mask[ 1 ] ? Dataset.DecodeToDatabaseString( q, DBCache.FID_sound, 5 ) : __default.m_sound;
				return temp;
			}
			return null;
		}

 		public static SoundCfg Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			SoundCfg ret = null;
		...
		const int VersionHash = 0x2c49a586;
		#endregion
	}
}
//EOF
