...
namespace Database {
	public partial class Equip_Inten_Part {
		const string DBName = "equip_inten_part";
		#region members
		private int m_id = 0;
		private float m_atk_mag = 0;
		private float m_atk_phy = 0;
		private float m_def_mag = 0;
		private float m_def_phy = 0;
		private float m_hpmax = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, Equip_Inten_Part> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Equip_Inten_Part>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public float atk_mag { get { return m_atk_mag; } }
		public float atk_phy { get { return m_atk_phy; } }
		public float def_mag { get { return m_def_mag; } }
		public float def_phy { get { return m_def_phy; } }
		public float hpmax { get { return m_hpmax; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static Equip_Inten_Part __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static Equip_Inten_Part() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Equip_Inten_Part ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Equip_Inten_Part> idCacheData {
		...
		static Equip_Inten_Part ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Equip_Inten_Part();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_atk_mag = __mask[ 0 ] ? q.GetFloat( DBCache.FID_atk_mag, 7 ) : __default.m_atk_mag;
				temp.m_atk_phy = __mask[ 1 ] ? q.GetFloat( DBCache.FID_atk_phy, 7 ) : __default.m_atk_phy;
				temp.m_def_mag = __mask[ 2 ] ? q.GetFloat( DBCache.FID_def_mag, 7 ) : __default.m_def_mag;
				temp.m_def_phy = __mask[ 3 ] ? q.GetFloat( DBCache.FID_def_phy, 7 ) : __default.m_def_phy;
				temp.m_hpmax = __mask[ 4 ] ? q.GetFloat( DBCache.FID_hpmax, 5 ) : __default.m_hpmax;
				return temp;
			}
			return null;
		}

 		public static Equip_Inten_Part Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Equip_Inten_Part ret = null;
		...
		const int VersionHash = 0x0ae96fd4;
		#endregion
	}
}
//EOF
