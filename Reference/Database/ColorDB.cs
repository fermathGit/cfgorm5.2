...
namespace Database {
	public partial class ColorDB {
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, ColorDB> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, ColorDB>();
		#endregion
		#region properties
		...
		static ColorDB() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( ColorDB ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, ColorDB> idCacheData {
		...
		static ColorDB ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new ColorDB();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_blue = __mask[ 0 ] ? q.GetInt( DBCache.FID_blue, 4 ) : __default.m_blue;
				temp.m_green = __mask[ 1 ] ? q.GetInt( DBCache.FID_green, 5 ) : __default.m_green;
				temp.m_red = __mask[ 2 ] ? q.GetInt( DBCache.FID_red, 3 ) : __default.m_red;
				return temp;
			}
			return null;
		}

 		public static ColorDB Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			ColorDB ret = null;
		...
		const int VersionHash = 0x5c7ad972;
		#endregion
	}
}
//EOF
