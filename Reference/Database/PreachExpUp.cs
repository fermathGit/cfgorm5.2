...
namespace Database {
	public partial class PreachExpUp {
		const string DBName = "preachEXP_up";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, PreachExpUp> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, PreachExpUp>();
		#endregion
		#region properties
		...
		static PreachExpUp __default = null;
		static int __hash = 0;
		...
		static PreachExpUp() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( PreachExpUp ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, PreachExpUp> idCacheData {
		...
		static PreachExpUp ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new PreachExpUp();
				...
				temp.m_dailyid = __mask[ 3 ] ? q.GetInt( DBCache.FID_dailyid, 7 ) : __default.m_dailyid;
				temp.m_expAddPer = __mask[ 4 ] ? q.GetInt( DBCache.FID_expAddPer, 9 ) : __default.m_expAddPer;
				temp.m_mapId = __mask[ 5 ] ? q.GetInt( DBCache.FID_mapId, 5 ) : __default.m_mapId;
				temp.m_moneyAddPer = __mask[ 6 ] ? q.GetInt( DBCache.FID_moneyAddPer, 11 ) : __default.m_moneyAddPer;
				return temp;
			}
			return null;
		}

 		public static PreachExpUp Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			PreachExpUp ret = null;
		...
		const int VersionHash = 0x0b994a66;
		#endregion
	}
}
//EOF
