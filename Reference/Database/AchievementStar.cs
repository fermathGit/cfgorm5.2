...
namespace Database {
	public partial class AchievementStar {
		const string DBName = "achievementStar";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, AchievementStar> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, AchievementStar>();
		#endregion
		#region properties
		...
		static AchievementStar __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static AchievementStar() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( AchievementStar ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, AchievementStar> idCacheData {
		...
		static AchievementStar ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new AchievementStar();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_item = __mask[ 0 ] ? q.GetInt( DBCache.FID_item, 4 ) : __default.m_item;
				temp.m_num = __mask[ 1 ] ? q.GetInt( DBCache.FID_num, 3 ) : __default.m_num;
				temp.m_star = __mask[ 2 ] ? q.GetInt( DBCache.FID_star, 4 ) : __default.m_star;
				return temp;
			}
			return null;
		}

 		public static AchievementStar Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			AchievementStar ret = null;
		...
		const int VersionHash = 0x7e3cd8c6;
		#endregion
	}
}
//EOF
