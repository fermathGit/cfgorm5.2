...
namespace Database {
	public partial class ChaosFightWeekReward {
		const string DBName = "Chaosbattle_Weekreward";
		#region members
		private int m_id = 0;
		...
		private int m_rankmax = 0;
		private int m_rankmin = 0;
		private int m_stage = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, ChaosFightWeekReward> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, ChaosFightWeekReward>();
		#endregion
		#region properties
		...
		static ChaosFightWeekReward __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static ChaosFightWeekReward() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( ChaosFightWeekReward ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, ChaosFightWeekReward> idCacheData {
		...
		static ChaosFightWeekReward ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new ChaosFightWeekReward();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_item1 = __mask[ 0 ] ? q.GetInt( DBCache.FID_item1, 5 ) : __default.m_item1;
				temp.m_item2 = __mask[ 1 ] ? q.GetInt( DBCache.FID_item2, 5 ) : __default.m_item2;
				temp.m_item3 = __mask[ 2 ] ? q.GetInt( DBCache.FID_item3, 5 ) : __default.m_item3;
				temp.m_item4 = __mask[ 3 ] ? q.GetInt( DBCache.FID_item4, 5 ) : __default.m_item4;
				temp.m_num1 = __mask[ 4 ] ? q.GetInt( DBCache.FID_num1, 4 ) : __default.m_num1;
				temp.m_num2 = __mask[ 5 ] ? q.GetInt( DBCache.FID_num2, 4 ) : __default.m_num2;
				temp.m_num3 = __mask[ 6 ] ? q.GetInt( DBCache.FID_num3, 4 ) : __default.m_num3;
				temp.m_num4 = __mask[ 7 ] ? q.GetInt( DBCache.FID_num4, 4 ) : __default.m_num4;
				temp.m_rankmax = __mask[ 8 ] ? q.GetInt( DBCache.FID_rankmax, 7 ) : __default.m_rankmax;
				temp.m_rankmin = __mask[ 9 ] ? q.GetInt( DBCache.FID_rankmin, 7 ) : __default.m_rankmin;
				temp.m_stage = __mask[ 10 ] ? q.GetInt( DBCache.FID_stage, 5 ) : __default.m_stage;
				return temp;
			}
			return null;
		}

 		public static ChaosFightWeekReward Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			ChaosFightWeekReward ret = null;
		...
		const int VersionHash = 0x6cd2ce6d;
		#endregion
	}
}
//EOF
