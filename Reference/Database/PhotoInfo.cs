...
namespace Database {
	public partial class PhotoInfo {
		const string DBName = "photo_image";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, PhotoInfo> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, PhotoInfo>();
		#endregion
		#region properties
		...
		static PhotoInfo __default = null;
		static int __hash = 0;
		...
		static PhotoInfo() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( PhotoInfo ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, PhotoInfo> idCacheData {
		...
		static PhotoInfo ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new PhotoInfo();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_p1 = __mask[ 0 ] ? Dataset.DecodeToDatabaseString( q, DBCache.FID_p1, 2 ) : __default.m_p1;
 		...
 		public static PhotoInfo Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			PhotoInfo ret = null;
		...
		const int VersionHash = 0x714c104d;
		#endregion
	}
}
//EOF
