...
namespace Database {
	public partial class Soul_Type {
		const string DBName = "soul_type";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Soul_Type> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Soul_Type>();
		#endregion
		#region properties
		...
		static Soul_Type __default = null;
		static int __hash = 0;
		...
		static Soul_Type() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Soul_Type ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Soul_Type> idCacheData {
		...
		static Soul_Type ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Soul_Type();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_ifhide = __mask[ 0 ] ? q.GetInt( DBCache.FID_ifhide, 6 ) : __default.m_ifhide;
				temp.m_name = __mask[ 1 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_name, 4 ) : __default.m_name;
				temp.m_type = __mask[ 2 ] ? q.GetInt( DBCache.FID_type, 4 ) : __default.m_type;
				return temp;
			}
			return null;
		}

 		public static Soul_Type Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Soul_Type ret = null;
		...
		const int VersionHash = 0x673236fc;
		#endregion
	}
}
//EOF
