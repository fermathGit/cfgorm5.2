...
namespace Database {
	public partial class Command {
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Command> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Command>();
		#endregion
		#region properties
		...
		static Command() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Command ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Command> idCacheData {
		...
		static Command ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Command();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_cmd = __mask[ 0 ] ? Dataset.DecodeToDatabaseString( q, DBCache.FID_cmd, 3 ) : __default.m_cmd;
				temp.m_desc = __mask[ 1 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_desc, 4 ) : __default.m_desc;
				temp.m_page = __mask[ 2 ] ? q.GetInt( DBCache.FID_page, 4 ) : __default.m_page;
		...
		const int VersionHash = 0x50ceb973;
		#endregion
	}
}
//EOF
