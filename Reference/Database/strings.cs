...
namespace Database {
	public partial class strings {
		const string DBName = "strings";
		#region members
		private string m_text = String.Empty;
		#endregion
		#region caches
		private static SKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<string, strings> m_idCache = new Common.Collections.Generic.Lit.Dictionary<string, strings>();
		#endregion
		#region properties
		...
		static strings() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( strings ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<string, strings> idCacheData {
		...
		static strings ReadOne( LuaDatabase.Query q, string id ) {
			if ( q.IsTable() ) {
				var temp = new strings();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_text = __mask[ 0 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_text, 4 ) : __default.m_text;
				return temp;
			}
			return null;
		}

 		public static strings Get( string id ) {
			strings ret = null;
		...
		const int VersionHash = 0x1cf82d15;
		#endregion
	}
}
//EOF
