...
namespace Database {
	public partial class Land_Reward {
		const string DBName = "land_reward";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Land_Reward> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Land_Reward>();
		#endregion
		#region properties
		...
		static Land_Reward() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Land_Reward ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Land_Reward> idCacheData {
		...
		static Land_Reward ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Land_Reward();
				...
				temp.m_key = __mask[ 3 ] ? q.GetInt( DBCache.FID_key, 3 ) : __default.m_key;
				temp.m_land = __mask[ 4 ] ? q.GetInt( DBCache.FID_land, 4 ) : __default.m_land;
				temp.m_num1 = __mask[ 5 ] ? q.GetInt( DBCache.FID_num1, 4 ) : __default.m_num1;
				temp.m_num2 = __mask[ 6 ] ? q.GetInt( DBCache.FID_num2, 4 ) : __default.m_num2;
				temp.m_num3 = __mask[ 7 ] ? q.GetInt( DBCache.FID_num3, 4 ) : __default.m_num3;
				temp.m_pic = __mask[ 8 ] ? q.GetInt( DBCache.FID_pic, 3 ) : __default.m_pic;
				return temp;
			}
			return null;
		}

 		public static Land_Reward Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Land_Reward ret = null;
		...
		const int VersionHash = 0x5192bfdb;
		#endregion
	}
}
//EOF
