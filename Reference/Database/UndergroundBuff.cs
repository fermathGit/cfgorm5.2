...
namespace Database {
	public partial class UndergroundBuff {
		const string DBName = "Underground_buff";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, UndergroundBuff> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, UndergroundBuff>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public int buff { get { return m_buff; } }
		public int dec { get { return m_dec; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static UndergroundBuff __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static UndergroundBuff() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( UndergroundBuff ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, UndergroundBuff> idCacheData {
		...
		static UndergroundBuff ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new UndergroundBuff();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_buff = __mask[ 0 ] ? q.GetInt( DBCache.FID_buff, 4 ) : __default.m_buff;
				temp.m_dec = __mask[ 1 ] ? q.GetInt( DBCache.FID_dec, 3 ) : __default.m_dec;
				return temp;
			}
			return null;
		}

 		public static UndergroundBuff Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			UndergroundBuff ret = null;
		...
		const int VersionHash = 0x2654540e;
		#endregion
	}
}
//EOF
