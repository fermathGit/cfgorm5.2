...
namespace Database {
	public partial class Equip_Addprop {
		const string DBName = "equip_addprop";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Equip_Addprop> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Equip_Addprop>();
		#endregion
		#region properties
		...
		static Equip_Addprop __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static Equip_Addprop() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Equip_Addprop ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Equip_Addprop> idCacheData {
		...
		static Equip_Addprop ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Equip_Addprop();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_Min = __mask[ 0 ] ? q.GetFloat( DBCache.FID_Min, 3 ) : __default.m_Min;
				temp.m_PropType = __mask[ 1 ] ? q.GetInt( DBCache.FID_PropType, 8 ) : __default.m_PropType;
				return temp;
			}
			return null;
		}

 		public static Equip_Addprop Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Equip_Addprop ret = null;
		...
		const int VersionHash = 0x25e4e04e;
		#endregion
	}
}
//EOF
