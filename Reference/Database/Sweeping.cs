...
namespace Database {
	public partial class Sweeping {
		const string DBName = "sweeping";
		#region members
		private int m_id = 0;
		private int m_cost_item = 0;
		private int m_cost_moneynum = 0;
		private int m_cost_moneytype = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Sweeping> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Sweeping>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public int cost_item { get { return m_cost_item; } }
		public int cost_moneynum { get { return m_cost_moneynum; } }
		public int cost_moneytype { get { return m_cost_moneytype; } }
		...
		static Sweeping __default = null;
		static int __hash = 0;
		...
		static Sweeping() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Sweeping ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Sweeping> idCacheData {
		...
		static Sweeping ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Sweeping();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_cost_item = __mask[ 0 ] ? q.GetInt( DBCache.FID_cost_item, 9 ) : __default.m_cost_item;
				temp.m_cost_moneynum = __mask[ 1 ] ? q.GetInt( DBCache.FID_cost_moneynum, 13 ) : __default.m_cost_moneynum;
				temp.m_cost_moneytype = __mask[ 2 ] ? q.GetInt( DBCache.FID_cost_moneytype, 14 ) : __default.m_cost_moneytype;
				temp.m_levelMax = __mask[ 3 ] ? q.GetInt( DBCache.FID_levelMax, 8 ) : __default.m_levelMax;
				temp.m_levelMin = __mask[ 4 ] ? q.GetInt( DBCache.FID_levelMin, 8 ) : __default.m_levelMin;
				temp.m_sub_id = __mask[ 5 ] ? q.GetInt( DBCache.FID_sub_id, 6 ) : __default.m_sub_id;
				temp.m_type = __mask[ 6 ] ? q.GetInt( DBCache.FID_type, 4 ) : __default.m_type;
				return temp;
			}
			return null;
		}

 		public static Sweeping Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Sweeping ret = null;
		...
		const int VersionHash = 0x5af31346;
		#endregion
	}
}
//EOF
