...
namespace Database {
	public partial class WingQuality {
		const string DBName = "wingQuality";
		#region members
		private int m_id = 0;
		private float m_att1 = 0;
		private int m_quality = 0;
		private int m_wingId = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, WingQuality> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, WingQuality>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public float att1 { get { return m_att1; } }
		...
		static WingQuality __default = null;
		static int __hash = 0;
		...
		static WingQuality() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( WingQuality ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, WingQuality> idCacheData {
		...
		static WingQuality ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new WingQuality();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_att1 = __mask[ 0 ] ? q.GetFloat( DBCache.FID_att1, 4 ) : __default.m_att1;
				temp.m_quality = __mask[ 1 ] ? q.GetInt( DBCache.FID_quality, 7 ) : __default.m_quality;
				temp.m_wingId = __mask[ 3 ] ? q.GetInt( DBCache.FID_wingId, 6 ) : __default.m_wingId;
				WingQualityEx.IsValid_upQualityCostItemes = __mask[ 2 ];
				if ( WingQualityEx.onLoadComplexField_upQualityCostItemes != null ) {
					WingQualityEx.onLoadComplexField_upQualityCostItemes( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static WingQuality Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			WingQuality ret = null;
		...
		const int VersionHash = 0x2cd6f47b;
		#endregion
	}

	public partial class WingQualityEx {
		public delegate void LoadComplexField( WingQuality data, LuaDatabase.Query q );
		internal static bool IsValid_upQualityCostItemes = true;
		static LoadComplexField OnLoadComplexField_upQualityCostItemes = null;
		public static LoadComplexField onLoadComplexField_upQualityCostItemes {
			get {
				return OnLoadComplexField_upQualityCostItemes;
			}
		}
	}
}
