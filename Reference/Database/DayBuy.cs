...
namespace Database {
	public partial class DayBuy {
		const string DBName = "daybuy";
		#region members
		private int m_id = 0;
		private int m_buyMoneyType = 0;
		private int m_buyMoneyValue = 0;
		private int m_day = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, DayBuy> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, DayBuy>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public int buyMoneyType { get { return m_buyMoneyType; } }
		public int buyMoneyValue { get { return m_buyMoneyValue; } }
		...
		static DayBuy() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( DayBuy ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, DayBuy> idCacheData {
		...
		static DayBuy ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new DayBuy();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_buyMoneyType = __mask[ 0 ] ? q.GetInt( DBCache.FID_buyMoneyType, 12 ) : __default.m_buyMoneyType;
				temp.m_buyMoneyValue = __mask[ 1 ] ? q.GetInt( DBCache.FID_buyMoneyValue, 13 ) : __default.m_buyMoneyValue;
				temp.m_day = __mask[ 2 ] ? q.GetInt( DBCache.FID_day, 3 ) : __default.m_day;
				temp.m_item = __mask[ 3 ] ? q.GetInt( DBCache.FID_item, 4 ) : __default.m_item;
				temp.m_num = __mask[ 4 ] ? q.GetInt( DBCache.FID_num, 3 ) : __default.m_num;
				DayBuyEx.IsValid_show_item = __mask[ 5 ];
				if ( DayBuyEx.onLoadComplexField_show_item != null ) {
					DayBuyEx.onLoadComplexField_show_item( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static DayBuy Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			DayBuy ret = null;
		...
		const int VersionHash = 0x20b3dfc7;
		#endregion
	}

	public partial class DayBuyEx {
		public delegate void LoadComplexField( DayBuy data, LuaDatabase.Query q );
		internal static bool IsValid_show_item = true;
		static LoadComplexField OnLoadComplexField_show_item = null;
		public static LoadComplexField onLoadComplexField_show_item {
			get {
				return OnLoadComplexField_show_item;
			}
		}
	}
}
