...
namespace Database {
	public partial class Battlefield_eternalDay {
		const string DBName = "battlefield_eternalDay";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Battlefield_eternalDay> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Battlefield_eternalDay>();
		#endregion
		#region properties
		...
		static Battlefield_eternalDay __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static Battlefield_eternalDay() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Battlefield_eternalDay ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Battlefield_eternalDay> idCacheData {
		...
		static Battlefield_eternalDay ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Battlefield_eternalDay();
				...
				temp.m_score = __mask[ 8 ] ? q.GetInt( DBCache.FID_score, 5 ) : __default.m_score;
				return temp;
			}
			return null;
		}

 		public static Battlefield_eternalDay Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Battlefield_eternalDay ret = null;
		...
		const int VersionHash = 0x0037a669;
		#endregion
	}
}
//EOF
