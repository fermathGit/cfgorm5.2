...
namespace Database {
	public partial class Offstringingame {
		const string DBName = "offstringingame";
		#region members
		private string m_offstring = String.Empty;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, Offstringingame> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Offstringingame>();
		#endregion
		#region properties
		public string offstring { get { return m_offstring; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static Offstringingame __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static Offstringingame() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Offstringingame ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Offstringingame> idCacheData {
		...
		static Offstringingame ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Offstringingame();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_offstring = __mask[ 0 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_offstring, 9 ) : __default.m_offstring;
				return temp;
			}
			return null;
		}

 		public static Offstringingame Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Offstringingame ret = null;
		...
		const int VersionHash = 0x57bc3261;
		#endregion
	}
}
//EOF
