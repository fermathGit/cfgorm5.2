...
namespace Database {
	public partial class BloomSetting {
		const string DBName = "bloom";
		#region members
		private int m_id = 0;
		private int m_blur_inter = 0;
		private int m_blursize = 0;
		private int m_down_sample = 0;
		private float m_intensity = 0;
		private float m_threshhold = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, BloomSetting> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, BloomSetting>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public int blur_inter { get { return m_blur_inter; } }
		public int blursize { get { return m_blursize; } }
		public int down_sample { get { return m_down_sample; } }
		public float intensity { get { return m_intensity; } }
		public float threshhold { get { return m_threshhold; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static BloomSetting __default = null;
		static int __hash = 0;
		...
		static BloomSetting() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( BloomSetting ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, BloomSetting> idCacheData {
		...
		static BloomSetting ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new BloomSetting();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_blur_inter = __mask[ 0 ] ? q.GetInt( DBCache.FID_blur_inter, 10 ) : __default.m_blur_inter;
				temp.m_blursize = __mask[ 1 ] ? q.GetInt( DBCache.FID_blursize, 8 ) : __default.m_blursize;
				temp.m_down_sample = __mask[ 2 ] ? q.GetInt( DBCache.FID_down_sample, 11 ) : __default.m_down_sample;
				temp.m_intensity = __mask[ 3 ] ? q.GetFloat( DBCache.FID_intensity, 9 ) : __default.m_intensity;
				temp.m_threshhold = __mask[ 4 ] ? q.GetFloat( DBCache.FID_threshhold, 10 ) : __default.m_threshhold;
				return temp;
			}
			return null;
		}

 		public static BloomSetting Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			BloomSetting ret = null;
		...
		const int VersionHash = 0x4e85758d;
		#endregion
	}
}
//EOF
