...
namespace Database {
	public partial class GuildBoss {
		const string DBName = "guild_boss";
		#region members
		private int m_id = 0;
		private int m_guildb_allrew = 0;
		private float m_guildb_aprew = 0;
		private float m_guildb_losrew = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, GuildBoss> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, GuildBoss>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public int guildb_allrew { get { return m_guildb_allrew; } }
		public float guildb_aprew { get { return m_guildb_aprew; } }
		public float guildb_losrew { get { return m_guildb_losrew; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static GuildBoss __default = null;
		static int __hash = 0;
		...
		static GuildBoss() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( GuildBoss ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, GuildBoss> idCacheData {
		...
		static GuildBoss ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new GuildBoss();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_guildb_allrew = __mask[ 0 ] ? q.GetInt( DBCache.FID_guildb_allrew, 13 ) : __default.m_guildb_allrew;
				temp.m_guildb_aprew = __mask[ 1 ] ? q.GetFloat( DBCache.FID_guildb_aprew, 12 ) : __default.m_guildb_aprew;
				temp.m_guildb_losrew = __mask[ 2 ] ? q.GetFloat( DBCache.FID_guildb_losrew, 13 ) : __default.m_guildb_losrew;
				GuildBossEx.IsValid_guildb_mongrow = __mask[ 3 ];
				if ( GuildBossEx.onLoadComplexField_guildb_mongrow != null ) {
					GuildBossEx.onLoadComplexField_guildb_mongrow( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static GuildBoss Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			GuildBoss ret = null;
		...
		const int VersionHash = 0x391b4829;
		#endregion
	}

	public partial class GuildBossEx {
		public delegate void LoadComplexField( GuildBoss data, LuaDatabase.Query q );
		internal static bool IsValid_guildb_mongrow = true;
		static LoadComplexField OnLoadComplexField_guildb_mongrow = null;
		public static LoadComplexField onLoadComplexField_guildb_mongrow {
			get {
				return OnLoadComplexField_guildb_mongrow;
			}
		}
	}
}
