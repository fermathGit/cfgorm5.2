...
namespace Database {
	public partial class PreachExp {
		const string DBName = "preachEXP";
		#region members
		private int m_id = 0;
		private int m_exp = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, PreachExp> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, PreachExp>();
		#endregion
		#region properties
		...
		static PreachExp __default = null;
		static int __hash = 0;
		...
		static PreachExp() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( PreachExp ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, PreachExp> idCacheData {
		...
		static PreachExp ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new PreachExp();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_exp = __mask[ 0 ] ? q.GetInt( DBCache.FID_exp, 3 ) : __default.m_exp;
				temp.m_level = __mask[ 1 ] ? q.GetInt( DBCache.FID_level, 5 ) : __default.m_level;
				return temp;
			}
			return null;
		}

 		public static PreachExp Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			PreachExp ret = null;
		...
		const int VersionHash = 0x3ab8a227;
		#endregion
	}
}
//EOF
