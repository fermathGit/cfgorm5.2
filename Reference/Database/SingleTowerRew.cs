...
namespace Database {
	public partial class SingleTowerRew {
		const string DBName = "single_tower_rew";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, SingleTowerRew> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, SingleTowerRew>();
		#endregion
		#region properties
		...
		static SingleTowerRew __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static SingleTowerRew() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( SingleTowerRew ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, SingleTowerRew> idCacheData {
		...
		static SingleTowerRew ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new SingleTowerRew();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_star = __mask[ 0 ] ? q.GetInt( DBCache.FID_star, 4 ) : __default.m_star;
				SingleTowerRewEx.IsValid_star_rew = __mask[ 1 ];
				if ( SingleTowerRewEx.onLoadComplexField_star_rew != null ) {
					SingleTowerRewEx.onLoadComplexField_star_rew( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static SingleTowerRew Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			SingleTowerRew ret = null;
		...
		const int VersionHash = 0x02def87a;
		#endregion
	}

	public partial class SingleTowerRewEx {
		public delegate void LoadComplexField( SingleTowerRew data, LuaDatabase.Query q );
		internal static bool IsValid_star_rew = true;
		static LoadComplexField OnLoadComplexField_star_rew = null;
		public static LoadComplexField onLoadComplexField_star_rew {
			get {
				return OnLoadComplexField_star_rew;
			}
		}
	}
}
