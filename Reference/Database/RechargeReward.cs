...
namespace Database {
	public partial class RechargeReward {
		const string DBName = "recharge_reward";
		#region members
		private int m_id = 0;
		...
		private int m_recharge = 0;
		private int m_type = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, RechargeReward> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, RechargeReward>();
		#endregion
		#region properties
		...
		public int recharge { get { return m_recharge; } }
		public int type { get { return m_type; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static RechargeReward __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static RechargeReward() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( RechargeReward ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, RechargeReward> idCacheData {
		...
		static RechargeReward ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new RechargeReward();
				...
				temp.m_item1 = __mask[ 1 ] ? q.GetInt( DBCache.FID_item1, 5 ) : __default.m_item1;
				temp.m_item10 = __mask[ 2 ] ? q.GetInt( DBCache.FID_item10, 6 ) : __default.m_item10;
				temp.m_item2 = __mask[ 3 ] ? q.GetInt( DBCache.FID_item2, 5 ) : __default.m_item2;
				temp.m_item3 = __mask[ 4 ] ? q.GetInt( DBCache.FID_item3, 5 ) : __default.m_item3;
				temp.m_item4 = __mask[ 5 ] ? q.GetInt( DBCache.FID_item4, 5 ) : __default.m_item4;
				temp.m_item5 = __mask[ 6 ] ? q.GetInt( DBCache.FID_item5, 5 ) : __default.m_item5;
				temp.m_item6 = __mask[ 7 ] ? q.GetInt( DBCache.FID_item6, 5 ) : __default.m_item6;
				temp.m_item7 = __mask[ 8 ] ? q.GetInt( DBCache.FID_item7, 5 ) : __default.m_item7;
				temp.m_item8 = __mask[ 9 ] ? q.GetInt( DBCache.FID_item8, 5 ) : __default.m_item8;
				temp.m_item9 = __mask[ 10 ] ? q.GetInt( DBCache.FID_item9, 5 ) : __default.m_item9;
				...
				temp.m_recharge = __mask[ 21 ] ? q.GetInt( DBCache.FID_recharge, 8 ) : __default.m_recharge;
				temp.m_type = __mask[ 22 ] ? q.GetInt( DBCache.FID_type, 4 ) : __default.m_type;
				return temp;
			}
			return null;
		}

 		public static RechargeReward Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			RechargeReward ret = null;
		...
		const int VersionHash = 0x5abc73b3;
		#endregion
	}
}
//EOF
