...
namespace Database {
	public partial class ItemMixGroup {
		const string DBName = "item_mixgroup";
		#region members
		private int m_id = 0;
		private string m_group_name = String.Empty;
		private int m_group_order = 0;
		private int m_group_type = 0;
		private int m_show = 0;
		private string m_type_name = String.Empty;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, ItemMixGroup> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, ItemMixGroup>();
		#endregion
		#region properties
		public int id { get { return m_id; } }
		public string group_name { get { return m_group_name; } }
		public int group_order { get { return m_group_order; } }
		public int group_type { get { return m_group_type; } }
		...
		static ItemMixGroup __default = null;
		static int __hash = 0;
		...
		static ItemMixGroup() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( ItemMixGroup ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, ItemMixGroup> idCacheData {
		...
		static ItemMixGroup ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new ItemMixGroup();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_group_name = __mask[ 0 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_group_name, 10 ) : __default.m_group_name;
				temp.m_group_order = __mask[ 1 ] ? q.GetInt( DBCache.FID_group_order, 11 ) : __default.m_group_order;
				temp.m_group_type = __mask[ 2 ] ? q.GetInt( DBCache.FID_group_type, 10 ) : __default.m_group_type;
				temp.m_show = __mask[ 3 ] ? q.GetInt( DBCache.FID_show, 4 ) : __default.m_show;
				temp.m_type_name = __mask[ 4 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_type_name, 9 ) : __default.m_type_name;
				return temp;
			}
			return null;
		}

 		public static ItemMixGroup Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			ItemMixGroup ret = null;
		...
		const int VersionHash = 0x4acd65ec;
		#endregion
	}
}
//EOF
