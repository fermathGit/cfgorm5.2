...
namespace Database {
	public partial class ExpIncrease {
		const string DBName = "exp_increase";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, ExpIncrease> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, ExpIncrease>();
		#endregion
		#region properties
		...
		static ExpIncrease __default = null;
		static int __hash = 0;
		...
		static ExpIncrease() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( ExpIncrease ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, ExpIncrease> idCacheData {
		...
		static ExpIncrease ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new ExpIncrease();
 		...
 		public static ExpIncrease Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			ExpIncrease ret = null;
		...
		const int VersionHash = 0x3b59ed66;
		#endregion
	}
}
//EOF
