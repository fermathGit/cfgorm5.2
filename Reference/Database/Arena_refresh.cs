...
namespace Database {
	public partial class Arena_refresh {
		const string DBName = "arena_refresh";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, Arena_refresh> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Arena_refresh>();
		#endregion
		#region properties
		...
		static Arena_refresh __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static Arena_refresh() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Arena_refresh ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Arena_refresh> idCacheData {
		...
		static Arena_refresh ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Arena_refresh();
 		...
 		public static Arena_refresh Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Arena_refresh ret = null;
		...
		const int VersionHash = 0x1e1f7fd8;
		#endregion
	}
}
//EOF
