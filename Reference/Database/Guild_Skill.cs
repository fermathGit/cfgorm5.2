...
namespace Database {
	public partial class Guild_Skill {
		const string DBName = "guild_skill";
		#region members
		private int m_id = 0;
		private int m_guildlevel = 0;
		private int m_skillicon = 0;
		private int m_skillid = 0;
		private int m_skilllearn = 0;
		private int m_skilllevel = 0;
		private string m_skillname = String.Empty;
		private int m_skillproperty_next = 0;
		private int m_skillproperty_number = 0;
		#endregion
		#region caches
		private static IKeys m_ids = null;
		private static Common.Collections.Generic.Lit.Dictionary<int, Guild_Skill> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, Guild_Skill>();
		#endregion
		#region properties
		...
		public int skilllearn { get { return m_skilllearn; } }
		public int skilllevel { get { return m_skilllevel; } }
		public string skillname { get { return m_skillname; } }
		public int skillproperty_next { get { return m_skillproperty_next; } }
		public int skillproperty_number { get { return m_skillproperty_number; } }
		#endregion
		#region method
		static LuaTableHandle __this = default( LuaTableHandle );
		static Guild_Skill __default = null;
		static int __hash = 0;
		...
		static Guild_Skill() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( Guild_Skill ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, Guild_Skill> idCacheData {
		...
		static Guild_Skill ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new Guild_Skill();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_guildlevel = __mask[ 0 ] ? q.GetInt( DBCache.FID_guildlevel, 10 ) : __default.m_guildlevel;
				temp.m_skillicon = __mask[ 1 ] ? q.GetInt( DBCache.FID_skillicon, 9 ) : __default.m_skillicon;
				temp.m_skillid = __mask[ 2 ] ? q.GetInt( DBCache.FID_skillid, 7 ) : __default.m_skillid;
				temp.m_skilllearn = __mask[ 3 ] ? q.GetInt( DBCache.FID_skilllearn, 10 ) : __default.m_skilllearn;
				temp.m_skilllevel = __mask[ 4 ] ? q.GetInt( DBCache.FID_skilllevel, 10 ) : __default.m_skilllevel;
				temp.m_skillname = __mask[ 5 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_skillname, 9 ) : __default.m_skillname;
				temp.m_skillproperty_next = __mask[ 6 ] ? q.GetInt( DBCache.FID_skillproperty_next, 18 ) : __default.m_skillproperty_next;
				temp.m_skillproperty_number = __mask[ 7 ] ? q.GetInt( DBCache.FID_skillproperty_number, 20 ) : __default.m_skillproperty_number;
				return temp;
			}
			return null;
		}

 		public static Guild_Skill Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			Guild_Skill ret = null;
		...
		const int VersionHash = 0x23fdaa61;
		#endregion
	}
}
//EOF
