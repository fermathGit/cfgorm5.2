...
namespace Database {
	public partial class UndergroundTask {
		const string DBName = "Underground_task";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, UndergroundTask> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, UndergroundTask>();
		#endregion
		#region properties
		...
		static UndergroundTask __default = null;
		static int __hash = 0;
		static bool __masked = false;

		static UndergroundTask() {
			LuaDatabase.datasetUnloadEvent += OnUnload;
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( UndergroundTask ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, UndergroundTask> idCacheData {
		...
		static UndergroundTask ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new UndergroundTask();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_des = __mask[ 0 ] ? Dataset.DecodeToLocalizedString( q, DBCache.FID_des, 3 ) : __default.m_des;
				temp.m_sort = __mask[ 2 ] ? q.GetInt( DBCache.FID_sort, 4 ) : __default.m_sort;
				temp.m_subtype = __mask[ 3 ] ? q.GetInt( DBCache.FID_subtype, 7 ) : __default.m_subtype;
				temp.m_type = __mask[ 4 ] ? q.GetInt( DBCache.FID_type, 4 ) : __default.m_type;
				temp.m_value = __mask[ 5 ] ? q.GetInt( DBCache.FID_value, 5 ) : __default.m_value;
				UndergroundTaskEx.IsValid_itemList = __mask[ 1 ];
				if ( UndergroundTaskEx.onLoadComplexField_itemList != null ) {
					UndergroundTaskEx.onLoadComplexField_itemList( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static UndergroundTask Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			UndergroundTask ret = null;
		...
		const int VersionHash = 0x1be0a044;
		#endregion
	}

	public partial class UndergroundTaskEx {
		public delegate void LoadComplexField( UndergroundTask data, LuaDatabase.Query q );
		internal static bool IsValid_itemList = true;
		static LoadComplexField OnLoadComplexField_itemList = null;
		public static LoadComplexField onLoadComplexField_itemList {
			get {
				return OnLoadComplexField_itemList;
			}
		}
	}
}
