...
namespace Database {
	public partial class DailyGroup {
		const string DBName = "daily_Group";
		#region members
		private int m_id = 0;
		...
		private static Common.Collections.Generic.Lit.Dictionary<int, DailyGroup> m_idCache = new Common.Collections.Generic.Lit.Dictionary<int, DailyGroup>();
		#endregion
		#region properties
		...
		static DailyGroup __default = null;
		static int __hash = 0;
		...
		static DailyGroup() {
			...
			if ( Load() ) {
				DBLoadedItems.MarkAutoLoaded( DBName, typeof( DailyGroup ) );
			}
		}
		...
		public static Common.Collections.Generic.Lit.Dictionary<int, DailyGroup> idCacheData {
		...
		static DailyGroup ReadOne( LuaDatabase.Query q, int id ) {
			if ( q.IsTable() ) {
				var temp = new DailyGroup();
				var __mask = DBCache.LoadCurrentFieldMask( q, __masked );
				temp.m_id = id;
				temp.m_atk_num = __mask[ 0 ] ? q.GetInt( DBCache.FID_atk_num, 7 ) : __default.m_atk_num;
				temp.m_count = __mask[ 1 ] ? q.GetInt( DBCache.FID_count, 5 ) : __default.m_count;
				temp.m_dailyId = __mask[ 2 ] ? q.GetInt( DBCache.FID_dailyId, 7 ) : __default.m_dailyId;
				temp.m_mapDataId = __mask[ 4 ] ? q.GetInt( DBCache.FID_mapDataId, 9 ) : __default.m_mapDataId;
				temp.m_maxLevel = __mask[ 5 ] ? q.GetInt( DBCache.FID_maxLevel, 8 ) : __default.m_maxLevel;
				temp.m_minLevel = __mask[ 6 ] ? q.GetInt( DBCache.FID_minLevel, 8 ) : __default.m_minLevel;
				DailyGroupEx.IsValid_itemshow = __mask[ 3 ];
				if ( DailyGroupEx.onLoadComplexField_itemshow != null ) {
					DailyGroupEx.onLoadComplexField_itemshow( temp, q );
				}
				return temp;
			}
			return null;
 		...
 		public static DailyGroup Get( int id ) {
			if ( id == 0 ) {
				return null;
			}
			DailyGroup ret = null;
		...
		const int VersionHash = 0x60d95951;
		#endregion
	}

	public partial class DailyGroupEx {
		public delegate void LoadComplexField( DailyGroup data, LuaDatabase.Query q );
		internal static bool IsValid_itemshow = true;
		static LoadComplexField OnLoadComplexField_itemshow = null;
		public static LoadComplexField onLoadComplexField_itemshow {
			get {
				return OnLoadComplexField_itemshow;
			}
		}
	}
}
